...
export function MessageBubble({ message, activePillQuestion }: { message: ChatUIMessage; activePillQuestion?: string }) {
  ...
  if (isUser) {
    ...
    const resumeUploadMatch = textContent.match(/^\[Resume upload: ([^\]]+)\]\s*([\s\S]*)$/);
    if (resumeUploadMatch) {
      const fileName = resumeUploadMatch[1];
      return (
        <div className="flex justify-end">
          <div className="flex max-w-[min(100%,320px)] items-center gap-2 rounded-2xl border border-[#c9d6ee] bg-[#f0f6ff] px-3 py-2.5 text-sm text-[#0f1114]">
